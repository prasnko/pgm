...
public:
	...
	static long double getDomainSize(const vector<Variable*>& variables) {
		long double domain_size = 1;
	...
	/*static inline LogDouble getUniformDistribution(const vector<Variable*>& variables,vector<int> &wcv)
	{
		vector<Variable*> removedVariables;
		for(vector<int>::iterator iter;iter!=wcv.end();iter++)
		{
			for (int i = 0; i < variables.size(); i++)
			{
				if(*iter==variables[i]->getValue())
				removedVariables.push_back(variables[i]);
			}
		}
		int totalDomainSize = getDomainSize(removedVariables);
		double u = (1/totalDomainSize);
